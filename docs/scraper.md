...
MediaDB enforces the rules on every write path — scrape writes, `UpsertMediaTags`/`UpsertMediaTitleTags`, the scanner's staging, user flags and deck membership, and the legacy `InsertTag`/`InsertTagType` — so a value that slips past a scraper is refused and logged at warn rather than stored. When the vocabulary changes between releases, the startup seeding pass removes stored types and values the new rules refuse, together with every link to them (`sqlPruneOffVocabularyTags`). Before removing them it moves what earlier releases stored as genres and series onto the vocabulary the way a scrape maps them now: the free-form `genre` and `gamegenre` values go through the MiSTer arcade category table (`arcadegenre`) and then the ScreenScraper genre table (`ssgenre`), and `gamefamily` values through the franchise lookup. Values none of them knows are dropped, as a scrape drops them.
