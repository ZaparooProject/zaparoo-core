...
Backup methods operate on portable full-device ZIP snapshots. Portable and cloud snapshots omit paired-client credentials; restoring one restarts Core and requires clients to pair again. MiSTer `retroachievements.cfg` files are also omitted because they contain plaintext account credentials. Mutating, listing, inspecting, and restoring methods require a localhost client or a paired admin client. Restore is refused while media is active.
...
**Data swapping.** On platforms that support it (currently MiSTer), the active profile also owns its save files and save states. Switching profiles makes that profile's data live; the shared profile's data is simply the default locations, so creating profiles never moves existing saves. On MiSTer, bind mounts redirect the live paths without moving, copying, or rewriting save data, and per-profile data lives in plain directories under `zaparoo/profiles/<profileId>/`. The mechanism follows main's own storage rules: a USB storage root (`device.bin`) is respected, and NAS-mounted saves (cifs mount over `saves/`) are layered on — the profile pool is created inside the share (`saves/.zaparoo-profiles/<profileId>/`) so saves stay centralized, and the user's mount is never touched.

The optional `/media/fat/retroachievements.cfg` used by odelot's custom RetroAchievements Main/core forks is also profile-owned when present. This is not an official MiSTer feature. First activation seeds `/media/fat/zaparoo/profiles/<profileId>/retroachievements.cfg` from the currently visible config, then bind-mounts that profile copy over the fixed SD-card path; MiSTer's USB save-storage selection does not move it. Shared and per-profile copies are intentionally excluded from local and cloud backup/restore because the file contains a plaintext RetroAchievements password.

A swap requested while a game is running is deferred until the game stops (the running game keeps the data it launched with, matching the playtime pinning rule). Progress and failures are reported by the [`profiles.data`](notifications.md#profilesdata) notification; the `profilesSwapData` setting turns swapping off. What never swaps: other core configs and input mappings (device properties), the Recently Played lists, arcade nvram, and computer-core disk images (saves live inside the shared image). Per-profile MiSTer INI setups need no data swap — write a combo card like `**profile:<switchId>||mister.ini:2` to switch profile and INI together. Deleting a profile never deletes its data directory. Note that profile data lives under the `zaparoo/` directory: deleting a Zaparoo install directory deletes profile saves and RetroAchievements configs with it.
