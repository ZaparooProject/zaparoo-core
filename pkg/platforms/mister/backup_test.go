...
func TestBackupDefinitions(t *testing.T) {
	...
	assert.Equal(t, filepath.Join(rootDir, "saves"), definitions[5].SourceRoot)
	assert.Equal(t, "saves", definitions[5].Category)
	assert.Equal(t, filepath.Join(rootDir, "savestates"), definitions[6].SourceRoot)
	assert.Equal(t, "savestates", definitions[6].Category)
	assert.Equal(t, []platforms.BackupPattern{{All: true}}, definitions[6].Include)
}
