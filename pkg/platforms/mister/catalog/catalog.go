...
// Package catalog contains dependency-light MiSTer core and media definitions.
//
// It deliberately excludes platform runtime concerns such as configuration,
// RBF discovery, hooks, launch command I/O, and media tracking so standalone
// MiSTer tools can reuse these definitions without pulling in Zaparoo services.
package catalog
...
func Groups() map[string][]Core {
	...
	for id, members := range canonical.Groups {
		...
		for i := range members {
			member := &members[i]
			// Group entries preserve their slot overrides but inherit scan
			// metadata from the matching canonical system.
