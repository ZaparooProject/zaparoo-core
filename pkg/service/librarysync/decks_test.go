...
func TestSyncDecks_BothSidesEditedAreMerged(t *testing.T) {
	...
	f.online.putDeck(remote)
	require.NoError(t, f.db.UserDB.UpdateDeckMeta("0123456789ab", "Weekend Plus", "", nil))
	require.NoError(t, f.db.UserDB.ReplaceDeckItems("0123456789ab", []database.DeckItem{
...
func TestSyncDecks_RejectedDeckWaitsForAChange(t *testing.T) {
	...
	require.NoError(t, f.db.UserDB.ReplaceDeckItems("0123456789ab", []database.DeckItem{scriptDeckItem("A", "**a")}))
	result = f.syncDecks(t)
	assert.Equal(t, 1, result.Pushed)
}
