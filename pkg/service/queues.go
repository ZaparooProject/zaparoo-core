...
func processTokenQueue(
	...
	for {
		...
		case t := <-itq:
			...
			// The one place a token's traits are settled. Every token carrying
			// script text passes through here, and everything downstream reads
			// this set rather than deriving its own, so a token's traits cannot
			// change once it starts running.
			if parseErr == nil {
