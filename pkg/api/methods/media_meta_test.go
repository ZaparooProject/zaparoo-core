...
func TestHandleMediaMeta_AliasTitle(t *testing.T) {
	t.Parallel()

	mockDB := testhelpers.NewMockMediaDBI()
	mediaPath := filepath.Join("roms", "nes", "mario-jp.nes")
	parentDir := filepath.Join("roms", "nes")
	...
	mockDB.On("GetMediaTagsByMediaDBID", mock.Anything, int64(1)).Return([]database.TagInfo{}, nil)
	mockDB.On("GetMediaTitleTagsByMediaTitleDBID", mock.Anything, int64(10)).
		Return([]database.TagInfo{{Tag: "region:japan", Type: "region"}}, nil)
	mockDB.On("GetMediaProperties", mock.Anything, int64(1)).Return([]database.MediaProperty{}, nil)
	mockDB.On("GetMediaTitleProperties", mock.Anything, int64(10)).Return([]database.MediaProperty{}, nil)
	mockDB.On("FindMediaTitleByDBID", mock.Anything, int64(20)).Return(&parentTitle, nil)
	mockDB.On("GetMediaTitleTagsByMediaTitleDBID", mock.Anything, int64(20)).
		Return([]database.TagInfo{{Tag: "developer:nintendo", Type: "developer"}}, nil)
	mockDB.On("GetMediaTitleProperties", mock.Anything, int64(20)).
		Return([]database.MediaProperty{{TypeTag: "property:description", Text: "Classic platformer"}}, nil)
	...
	assert.Equal(t, "super-mario-bros", resp.Media.Title.Slug)
	assert.Equal(t, "Super Mario Bros", resp.Media.Title.Name)
	assert.Len(t, resp.Media.Title.Tags, 1)
	assert.Equal(t, "developer:nintendo", resp.Media.Title.Tags[0].Tag)
	assert.Contains(t, resp.Media.Title.Properties, "property:description")

	require.NotNil(t, resp.Media.AliasTitle)
	assert.Equal(t, "super-mario-bros-j", resp.Media.AliasTitle.Slug)
	assert.Equal(t, "Super Mario Bros. (Japan)", resp.Media.AliasTitle.Name)
	assert.Len(t, resp.Media.AliasTitle.Tags, 1)
	assert.Equal(t, "region:japan", resp.Media.AliasTitle.Tags[0].Tag)
	mockDB.AssertExpectations(t)
}

func TestHandleMediaMeta_CanonicalTitle_NoAliasTitle(t *testing.T) {
	t.Parallel()
	...
	row := &database.MediaFullRow{
		...
		Title:  database.MediaTitle{DBID: 10, Slug: "super-mario-bros", Name: "Super Mario Bros", ParentDBID: 0},
		System: database.System{DBID: 100, SystemID: "NES", Name: "NES"},
	}
	...
	require.True(t, ok)
	assert.Equal(t, "super-mario-bros", resp.Media.Title.Slug)
	assert.Nil(t, resp.Media.AliasTitle)
	mockDB.AssertExpectations(t)
}
