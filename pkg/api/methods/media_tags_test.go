...
func TestCapTagsByCategory_TieBreakByTagAsc(t *testing.T) {
	t.Parallel()
	tags := []database.TagInfo{
		{Type: "credit", Tag: "Zebra", Count: 5},
		{Type: "credit", Tag: "Alpha", Count: 5},
		{Type: "credit", Tag: "Middle", Count: 5},
	}
...
func TestCapTagsByCategory_MultiCategory(t *testing.T) {
	...
	tags := []database.TagInfo{
		...
		{Type: "credit", Tag: "c4", Count: 7},
		{Type: "credit", Tag: "c5", Count: 6},
	}
	result := capTagsByCategory(tags, 2)
	assert.Len(t, result, 2, "credit capped at 2")
...
func TestCapTagsByCategory_CategoryOrderPreserved(t *testing.T) {
	...
	tags := []database.TagInfo{
		...
		{Type: "region", Tag: "usa", Count: 1},
		{Type: "credit", Tag: "Alice", Count: 1},
	}
	result := capTagsByCategory(tags, 100)
	assert.Len(t, result, 3)
	assert.Equal(t, "year", result[0].Type, "first-seen type comes first")
	assert.Equal(t, "region", result[1].Type)
	assert.Equal(t, "credit", result[2].Type)
}

func TestCapTagsByCategory_TaxonomyTypesNotCapped(t *testing.T) {
	t.Parallel()
	tags := make([]database.TagInfo, 0, 150)
	for i := range 150 {
		tags = append(tags, database.TagInfo{
			Type:  "year",
			Tag:   fmt.Sprintf("19%02d", i%70+50),
			Count: int64(150 - i),
		})
	}
	result := capTagsByCategory(tags, 100)
	assert.Len(t, result, 150, "year tags should not be capped — finite taxonomy type")
}
