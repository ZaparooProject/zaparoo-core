...
func TestRecomputeSystemDisambiguation_LoneVariantAlwaysDisambiguates(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "hacked dump sub-variant", tags: map[string]string{"dump": "hacked:ffe"}, want: "dump"},
		{name: "modified dump", tags: map[string]string{"dump": "modified"}, want: "dump"},
...
// canonical value of the types the rule names, plus the hacked sub-variants.
func TestGameVariantTagSQLPredicate_AgreesWithGo(t *testing.T) {
	...
	assert.True(t, matched[pair{"dump", "hacked:ffe"}])
	assert.False(t, matched[pair{"dump", "hackedx"}])
}
